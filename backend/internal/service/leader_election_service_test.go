...
func TestLeaderElectionRunner(t *testing.T) {
	t.Run("standby does not run leader services while lock is held", func(t *testing.T) {
		...
		runner := newTestLeaderElectionRunner(standbyLock, func(ctx context.Context) ([]servicerunner.Service, func(), error) {
			close(called)
			return nil, nil, nil
		})
	...
	t.Run("standby becomes leader after current leader releases lock", func(t *testing.T) {
		...
		leaderRunner := newTestLeaderElectionRunner(leaderLock, func(ctx context.Context) ([]servicerunner.Service, func(), error) {
			return []servicerunner.Service{func(ctx context.Context) error {
				leaderOnce.Do(func() { close(leaderStarted) })
				<-ctx.Done()
				return ctx.Err()
			}}, nil, nil
		})
		standbyRunner := newTestLeaderElectionRunner(standbyLock, func(ctx context.Context) ([]servicerunner.Service, func(), error) {
			return []servicerunner.Service{func(ctx context.Context) error {
